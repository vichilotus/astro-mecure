    ...
    <li key={slug}>
      <a
        className={twMerge(
          ...
          isActive &&
            "before:absolute before:left-1 before:top-1/2 before:-translate-y-1/2 before:w-1 before:h-1 before:bg-primary-500 before:rounded-full",
...
export default function Toc({ toc }: TocProps) {
  ...
  const activeAncestors = active
    ? findAncestors(toc, active).map((heading) => heading.slug)
    : [];
  return (
    <nav className="text-sm plate-bg border-highlight p-3 rounded-xl">
      <h2 className="font-bold text-lg flex items-center mb-1">
