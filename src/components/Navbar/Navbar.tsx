...
interface CollapseReducerState {
  open: boolean;
  node: React.ReactNode;
}

interface SetCollapseOpenAction {
  type: ActionTypes.setCollapseOpen;
  open: boolean;
}

interface SetCollapseNodeAction {
  type: ActionTypes.setCollapseNode;
  node: React.ReactNode;
}

type CollapseReducerAction = SetCollapseOpenAction | SetCollapseNodeAction;

function collapseReducer(
  states: CollapseReducerState,
  action: CollapseReducerAction
): CollapseReducerState {
  switch (action.type) {
...
const CollapseDispatchContext = createContext<React.Dispatch<CollapseReducerAction> | null>(
  null
);

export interface NavbarLogoProps extends React.PropsWithChildren {}
const NavbarLogo = withPolymorphic<"div", NavbarLogoProps>(
  forwardRef(({ className, ...rest }, ref: React.Ref<HTMLDivElement>) => {
    return (
      <Polymorphic
        ref={ref}
        className={twMerge(
...
const NavbarContent = forwardRef(
  (
    { variant = "underline", className, ...rest }: NavbarContentProps,
    ...
    return (
      <ContentConfigContext.Provider value={config}>
        <div
          ref={ref}
          className={twMerge("flex flex-nowrap items-center h-full", className)}
          {...rest}
        />
...
const NavbarItem = withPolymorphic<"div", NavbarItemProps>(
  forwardRef(
    (
      { isActive = false, activeClassName, className, ...rest },
      ref: React.Ref<HTMLDivElement>
    ) => {
      ...
      if (context === null) {
        throw new Error(
          "<Navbar.Item /> should be wrapped by <Navbar.Content />."
        );
      }
      return (
        <Polymorphic
          ...
          className={twMerge(
            ...
            isActive && "font-bold",
            isActive && context.variant === "underline"
              ? "h-full text-primary-500 after:absolute after:inset-x-0 after:bottom-0 after:h-1 after:rounded-full after:bg-primary-500"
              : null,
            isActive && context.variant === "highlight"
              ? "text-primary-600 dark:text-primary-400 rounded-full bg-primary-400/30 dark:bg-primary-600/30"
              : null,
            isActive && context.variant === "filled"
              ? "text-gray-100 rounded-full bg-primary-500 dark:text-gray-800 dark:bg-gray-100"
              : null,
...
export interface NavbarCollapseTriggerProps
  extends React.PropsWithChildren<React.ComponentPropsWithoutRef<"button">> {}

const NavbarCollapseTrigger = forwardRef<
  HTMLButtonElement,
  NavbarCollapseTriggerProps
>(({ className, children, ...rest }, ref) => {
  ...
  return (
    <button ref={ref} className={className} {...rest} onClick={handleClick}>
      {children}
    </button>
  );
});
...
const Navbar = forwardRef<HTMLElement, NavbarProps>(
  (
    ...
    const shouldTransparent =
      transparentOnTop && isTop && states.open === false;
    // no render when onShowChange changed
    const handleShowChange = useCallbackRef(onShowChange);
    useEffect(() => {
      handleShowChange?.(!shouldHide);
    }, [shouldHide, handleShowChange]);
    ...
    const [collapseContentRef, collapseContentSize] =
    ...
    return (
      <>
        {position === "floating" && <div className="pb-[3.75rem]" />}
        <NoSSR>
          <animated.nav
            ref={ref}
            className={twMerge(
              ...
              <CollapseDispatchContext.Provider value={dispatch}>
                ...
                >
                  ...
                  {states.node && (
                    ...
                    >
                      <div ref={collapseContentRef}>{states.node}</div>
                    </animated.div>
                  )}
                </div>
