...
const edgeClass =
  "absolute from-background-light dark:from-background-dark to-transparent z-10 pointer-events-none";
...
export default function ScrollArea({
  ...
  return (
    ...
    >
      ...
      {edgeShadow && !useMask && !isAtTop && (
        <div
          className={twMerge(
            "top-0 h-8 inset-x-0 bg-gradient-to-b",
            edgeClass,
            edgeClassName
          )}
        />
      )}
      {edgeShadow && !useMask && !isAtBottom && (
        <div
          className={twMerge(
            "bottom-0 h-8 inset-x-0 bg-gradient-to-t",
            edgeClass,
            edgeClassName
          )}
        />
      )}
      {edgeShadow && !useMask && !isAtLeft && (
        <div
          className={twMerge(
            "left-0 w-8 inset-y-0 bg-gradient-to-r",
            edgeClass,
            edgeClassName
          )}
        />
      )}
      {edgeShadow && !useMask && !isAtRight && (
        <div
          className={twMerge(
            "right-0 w-8 inset-y-0 bg-gradient-to-l",
            edgeClass,
            edgeClassName
          )}
        />
