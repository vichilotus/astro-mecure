...
function itemsReducer(state: HTMLElement[], action: Action) {
  switch (action.type) {
    case ActionTypes.RegisterItem:
      return state.includes(action.element)
        ? [...state]
...
function HorizontalItem({ children, className, ...rest }: HorizontalItemProps) {
...
function IconButton({ className, icon, children, ...rest }: IconButtonProps) {
  return (
    <button
      tabIndex={-1}
      className={twMerge(
...
function findLeftOverflowItem(
  items: HTMLElement[],
  container: HTMLElement,
  padding: number,
) {
  const containerRect = container.getBoundingClientRect();
  const _items = items.sort(
...
function findRightOverflowItem(
  items: HTMLElement[],
  container: HTMLElement,
  padding: number,
) {
  ...
  return _items.find((item) => {
    ...
    return (
      ...
      containerRect.x + containerRect.width - padding
    );
  });
}
...
function Horizontal({
  ...
  const handleLeftArrowBtnClick = useCallback(
  ...
  const handleRightArrowBtnClick = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      if (ref.current) {
        const padding = 30;
