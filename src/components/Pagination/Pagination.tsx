...
const Pagination = forwardRef<HTMLDivElement, PaginationProps>(
  (
    ...
    const itemClassName =
      "rounded-md h-8 leading-8 text-center min-w-[2rem] select-none";

    const items = range.map((pageNumber, index) =>
      isLink &&
      index < pageUrls.length &&
      pageNumber !== "dots" &&
      pageNumber !== active ? (
        <a
          key={String(pageNumber) !== "dots" ? String(pageNumber) : `dots-${index}`}
          href={pageUrls[index]}
          className={twMerge(
        ...
        <div
          key={pageNumber !== "dots" ? pageNumber : `dots-${index}`}
          className={twMerge(
            pageNumber !== "dots" && "cursor-pointer px-2 text-sm",
            pageNumber !== "dots" &&
              pageNumber !== active &&
              "plate-bg plate-shadow border-highlight",
            pageNumber === active &&
              "!bg-primary-500 text-white shadow-primary-800/50 shadow-lg",
            itemClassName
          )}
          aria-current={pageNumber === active ? "page" : undefined}
          ...
          onKeyDown={
            pageNumber !== "dots" ? (e) => e.key === 'Enter' && setPage(pageNumber) : undefined
          }
        >
          {pageNumber === "dots" ? <IconDots className="inline" /> : pageNumber}
        </div>
      )
    );

    const BtnComponent = isLink ? "a" : "button";
    const prevEdgeComponent =
      active === 1 ? (
        <div
          aria-disabled="true"
          className={twMerge(
        ...
        >
          <IconChevronsLeft className="inline" />
        </div>
      ) : (
        <BtnComponent
    ...
    const prevControlComponent =
      active === 1 ? (
        ...
        <BtnComponent
          href={
            isLink && pageUrls.length && active - 2 >= 0
              ? `${pageUrls[active - 2]}`
              : undefined
          }
    ...
    const nextEdgeComponent =
      active === total ? (
        ...
        <BtnComponent
          href={
            isLink && pageUrls.length && active < pageUrls.length
              ? `${pageUrls[active]}`
              : undefined
          }
    ...
    const nextControlComponent =
      active === total ? (
        ...
        >
          <IconChevronsRight className="inline" />
        </div>
      ) : (
        <BtnComponent
          href={
            isLink && pageUrls.length
