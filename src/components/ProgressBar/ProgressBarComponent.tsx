...
const ProgressBar = forwardRef<ProgressBarRef>(
  (
    { progress, style, className, onChange, ...rest }: ProgressBarProps,
    ref,
  ) => {
    ...
    useImperativeHandle(ref, () => {
      return {
        ...
        add(delta: number) {
          setProgressCtrl(
            progressCtrl + delta > 100 ? 100 : progressCtrl + delta,
          );
        },
        reset() {
          api.set({
    ...
    }, [progressCtrl, api.set, setProgressCtrl]);

    return progressCtrl > 0 ? (
      <animated.div
        style={{
          ...style,
          ...spring,
