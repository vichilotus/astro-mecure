...
function h(
  ...
  const filteredChildren =
    children?.filter<MdastContent>((child): child is MdastContent => {
...
function toMdast(node: HastContent) {
  ...
  if (node.type === "element") {
    ...
    for (const child of node.children.map(toMdast)) {
      if (child) {
...
function htmlTemplate(
  ...
  return h(
    "div",
    {
      class: twMerge("my-4 rounded-lg px-4 py-2", config.class),
    },
    [
      h(
        "div",
