...
export default function Footer({
  ...
  return (
    ...
    >
      <div
        {...rest}
        className={twMerge("text-sm md:space-y-2 w-full", containerClassName)}
      >
        {/* Links */}
        {links && (
          <div className="space-x-3">
            {links.map((link) => (
              <a
                href={link.url}
                key={link.url}
                target={isUnderBaseUrl(link.url) ? "_self" : "_blank"}
              >
                {link.label}
              </a>
            ))}
        ...
        {declarations && (
          <div className="space-x-3">
            {declarations.map((decl) => (
              <span key={decl?.toString() ?? ''}>{decl}</span>
            ))}
          </div>
        )}
        {/* Site Info */}
          ...
          {generator && (
            <span>
              ...
              <a href="https://astro.build/" target="_blank" rel="noreferrer">
                <AstroLogo className="inline" />
                Astro
              </a>{" "}
              · Designed By Takashi Kindaichi.
            </span>
          )}
          {rss && (
            <a
              ...
              rel="noreferrer"
            >
              <IconRss className="inline align-[-3px] mr-1" />
              RSS Feed
            </a>
          )}
          {sitemap && (
            <a
              ...
              rel="noreferrer"
            >
              <IconSitemap className="inline align-[-3px] mr-1" />
              Sitemap
            </a>
          )}
        </div>
