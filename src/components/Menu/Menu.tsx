...
export interface MenuConfig
  extends Array<MenuSubItemConfig | MenuLinkItemConfig> {}
...
const DropdownCascadeSub = ({ menu, children }: DropdownCascadeSubProps) => {
  return (
    <DropdownMenu.Sub>
      ...
      <DropdownMenu.Portal>
        ...
        >
          {menu.map((subItem, index) => {
            if ("url" in subItem) {
              return (
                <DropdownMenu.Item
                  ...
                  className={twMerge(
                    itemClassName,
                    "pl-[1.625rem] pr-2 py-1.5 rounded"
                  )}
                  key={subItem.label}
                >
                  <a href={subItem.url}>
                    {subItem.icon && (
                      <Icon
                        ...
                        width={16}
                        height={16}
                        className="absolute left-0 mx-1"
                      />
                    )}
                    {subItem.label}
                  </a>
            ...
            return (
              <DropdownCascadeSub menu={subItem.children} key={subItem.label}>
                <div
                  className={twMerge(
                    itemClassName,
                    "pl-[1.625rem] pr-1 py-1.5 rounded"
                  )}
                >
                  {subItem.icon && (
                    <Icon
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          className={twMerge(
            ...
            size === "loose"
              ? "p-2 rounded-lg min-w-[14rem]"
              : size === "compact"
                ? "p-1 rounded-md min-w-[8rem] text-xs leading-3"
              ...
              return (
                <DropdownMenu.Item
                  ...
                  className={twMerge(
                    itemClassName,
                    size === "loose"
                      ? "pl-8 pr-2 py-2.5 rounded-md"
                      : size === "compact"
                        ? "pl-6 pr-2 py-1 rounded"
              ...
              <DropdownCascadeSub menu={subItem.children} key={subItem.label}>
                <div
                  className={twMerge(
                    itemClassName,
                    size === "loose"
                      ? "pl-8 pr-1 py-2.5 rounded-md"
                      : size === "compact"
                        ? "pl-6 pr-1 py-1 rounded"
